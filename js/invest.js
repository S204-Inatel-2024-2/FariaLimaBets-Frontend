...
function calculateInvestment(stockSymbol) {
    ...
    const investmentAmount = parseFloat(document.getElementById(`amount_${stockSymbol}`).value);

    if (investmentAmount > 0) {
        const unitsBought = investmentAmount / stockPrice;
        document.getElementById(`units_${stockSymbol}`).innerText = ` | Ações compradas: ${unitsBought.toFixed(4)}`;
    } else {
...
function invest(stockSymbol) {
    ...
    listItem.innerText = `Investido $${investmentAmount.toFixed(2)} em ${stockSymbol} (Preço: $${stockPrice.toFixed(2)}, Ações compradas: ${unitsBought.toFixed(4)})`;
